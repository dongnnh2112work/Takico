...
# Runtime files beside Play Takico.command (not inside _takico)
...
if [[ ! -x "$SERVER_BIN" ]]; then
  alert "Không chạy được takico-server. Chuột phải Play Takico.command → Open; hoặc: xattr -cr \"$RUNTIME_DIR\""
  exit 1
fi
